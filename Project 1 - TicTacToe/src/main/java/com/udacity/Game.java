...
public class Game {
    ...
    public String checkGameWinner(char [][]grid) {
        // counter's for keeping track of Diagonal 'X' & 'O' & looking for Tie
        int counterTie = 0;
        int counterXDiagonal = 0;
        int counterODiagonal = 0;
        // looping through Nested loop
        for (int i = 0; i < grid.length; i++) {
            // counter's for keeping track of horizontal/vertical 'X' & 'O'
            int counterOHorizontal = 0;
            int counterOVertical = 0;
            int counterXHorizontal = 0;
            int counterXVertical = 0;
            for (int j = 0; j < grid.length; j++) {
                // checking if we have 3 'O' or 'X' Vertically
                if (grid[i][j] == 'o') {
                    counterOVertical++;
                ...
                // checking if we have 3 'O' or 'X' horizontally
                if (grid[j][i] == 'o') {
                    counterOHorizontal++;
                    if (counterOHorizontal == 3) {
                        return "O Wins";
                    }
                } else if (grid[j][i] == 'x') {
                    counterXHorizontal++;
                    if (counterXHorizontal == 3) {
                        return "X Wins";
                    }
                }
                // Checking 'X' & 'O' diagonally right to left side of the grid
                if (i == j && grid[i][j] == 'o'){
                    counterODiagonal++;
                    if (counterODiagonal == 3){
                        return "O Wins";
                    }
                }else if (i == j && grid[i][j] == 'x'){
                    counterXDiagonal++;
        ...
        // checking 'X' & 'O' diagonally left to right side of the grid
        if ((grid[0][2] == 'o') && (grid[1][1] == 'o') && (grid[2][0] == 'o')){
            return "O Wins";
        } else if ((grid[0][2] == 'x' && grid[1][1] == 'x' && grid[2][0] == 'x')){
            return "X Wins";
        }
        return "None"; // returning None if game is still continued
    }
